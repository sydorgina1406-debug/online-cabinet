...
<div key={i} className={rounded-full shadow-sm w-full h-full ${pos}} style={{ backgroundColor: dotColor }}>
))}
...
return <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, fontWeight: 400 }}>{sec}с;
}
...
else if (d.id === 'timer_state') { setSessionTimer(d.data()); }
else if (!d.id.startsWith('')) cards.push({ id: d.id, ...d.data() });
});
...
try {
const elementsToSave = cardsOnTable.filter(c => c.id !== '_settings' && c.id !== '_dice_state' && c.id !== '_dice_type' && c.id !== 'library_state' && !c.id.startsWith(''));
...
<h3 className="text-lg font-black mb-4 text-center" style={{ color: COLORS.ink }}>{customDialog.title}
...
<button onClick={() => { customDialog.onCancel(); setCustomDialog(null); }} className="flex-1 py-3 font-bold rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors">Отмена
<button onClick={() => { const val = customDialog.type === 'prompt' ? document.getElementById('dialog-input').value : true; customDialog.onOk(val); setCustomDialog(null); }} className="flex-1 py-3 font-bold rounded-xl text-white transition-colors" style={{ backgroundColor: COLORS.plum }}>Ок
